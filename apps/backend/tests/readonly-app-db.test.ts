...
describe('runScopedAppDbQuery', () => {
	beforeEach(async () => {
		// Clean any leftovers from a previously aborted run (child -> parent, cascade-independent).
		await db.delete(chat).where(inArray(chat.id, ['ro-chat-a', 'ro-chat-b']));
	...
	it('returns only the scoped project rows', async () => {
		const { rows } = await runScopedAppDbQuery(PROJECT_A, 'SELECT id, title FROM v_chat');
		expect(rows.map((r) => r.id)).toEqual(['ro-chat-a']);
	});

	it('does not leak another project', async () => {
		const { rows } = await runScopedAppDbQuery(PROJECT_B, 'SELECT id FROM v_chat');
		expect(rows.map((r) => r.id)).toEqual(['ro-chat-b']);
	});

	it('does not expose PII columns (email is not on the view)', async () => {
		await expect(runScopedAppDbQuery(PROJECT_A, 'SELECT email FROM v_chat')).rejects.toThrow();
	});
