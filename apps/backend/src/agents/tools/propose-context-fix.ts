...
function resolveEditTarget(
	...
	linkedRepos: LinkedContextRepo[],
	allowContextEdits: boolean,
): Pick<ProposedEdit, 'targetRepoFullName' | 'targetRepoBranch' | 'targetPath'> {
	...
	const linkedRepo = linkedRepos.find((repo) => normalized.startsWith(`${normalizePath(repo.contextPath)}/`));
	if (!linkedRepo) {
		throw new Error(
	...
	const targetPath = normalized.slice(`${normalizePath(linkedRepo.contextPath)}/`.length);
	if (!targetPath) {
		throw new Error(`"${filePath}" points at a repository folder, not a file.`);
	}

	return {
		targetRepoFullName: linkedRepo.repoFullName,
		targetRepoBranch: linkedRepo.branch,
		targetPath,
	};
}
